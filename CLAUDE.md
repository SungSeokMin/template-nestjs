...
### 2. 플랜 작성, 자체 Adversarial 리뷰, 컨펌

- 플랜 모드에서 관련 코드를 탐색하고 구체적인 구현 계획을 작성한다.
- 계획에는 예상 에러 포인트(타입 불일치, null 가능성, 트랜잭션 경계, 권한 누락 등)를 함께 정리한다.
- 계획 작성 후, **adversarial reviewer 역할로 전환하여** 아래 항목을 스스로 검토하고 결과를 계획에 반영한다:
  - 논리적 허점, 누락된 엣지 케이스, 보안 취약점, 타입/트랜잭션 위험
- 최종 계획과 검토 요약을 사용자에게 함께 제시한다.
- **명시적인 승인을 받은 후에만** 코드를 작성한다.

### 3. 구현

- 2단계에서 승인된 범위만 구현한다. 임의로 범위를 확장하지 않는다.
- 만약 현재 브랜치가 dev 인 경우, 적절한 이름의 브랜치를 만들어서 구현한다. (`type/scope_요약`)
- 이 파일의 아키텍처 규칙(에러 처리, 인증, Prisma 컨벤션 등)을 모두 준수한다.

### 4. 자체 코드 검토 및 Codex 리뷰 (에러 포인트 점검)

구현 직후, 아래 항목을 코드에서 직접 확인하고 발견된 문제는 즉시 수정한다.
사용자에게 `/codex:review` 실행을 요청한다. (Claude는 직접 호출 불가 — 사용자가 직접 입력해야 한다.)
...
- Line length: 120 (biome.json 기준)
- Biome linter + formatter 사용
...
신규 프로젝트의 기본값은 Vitest.

## 에러 처리

`BusinessException` (`HttpException` 확장)을 사용하고, 에러 코드는 [`src/common/exceptions/error-code.enum.ts`](src/common/exceptions/error-code.enum.ts)의 `ErrorCode`에서 가져온다. `HttpException`을 직접 throw하지 않는다.
